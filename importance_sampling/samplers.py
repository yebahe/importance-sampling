...
class BaseSampler(object):
    ...
    def sample(self, batch_size):
        ...
        idxs1, scores, xy = self._get_samples_with_scores(batch_size) # 包含阈值判断
...
class ModelSampler(BaseSampler):
    ...
    def _get_samples_with_scores(self, batch_size):
        ...
        # 从范围 [0, self.N) 中随机选择 self.large_batch 个整数，作为索引值
        idxs = np.random.choice(self.N, self.large_batch)
        # 通过索引值获取数据
        x, y = self.dataset.train_data[idxs]

        end_time = time.time()
        signal("is.time").send(end_time - start_time,type='get_B')
        # 评分
