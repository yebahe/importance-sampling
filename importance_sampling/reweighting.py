...
class BiasedReweightingPolicy(ReweightingPolicy):
    ...
    def sample_weights(self, idxs, scores):
        ...
        s = scores[idxs]
        '''按照论文所述：'''
        '''w = 1/(B*gi) 即 1/(N*gi) '''# i是重要性抽样的选中样本序号
        '''又因为 gi∝Gi 所以 w = 1/(N*Gi) '''# Gi为gi对应的上界
        '''由 Gi = s/scores.sum() '''# Gi通过公式20计算
        '''得到结果：'''
        w = scores.sum() / N / s  
         # 当k=1时，w_hat = w
        w_hat = w**self.k
        w_hat *= w.dot(s) / w_hat.dot(s) #.dot()为点乘

        return w_hat[:, np.newaxis] # 将结果转化为列向量，并额外增加一列
...
class ExternalReweighting(Layer):
    ...
    def call(self, x):
        return K.stop_gradient(x[1]) # 标记张量为不可微分，阻止其在keras框架中的梯度反向传播(即不可使用K.gradient计算其梯度)
